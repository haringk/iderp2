...
def apply_global_minimums_server_side(doc, method=None):
    ...
    use_global_minimums = frappe.db.get_single_value("Selling Settings", "use_global_minimums") or True
...
def apply_aggregated_minimums(doc, customer_group):
    ...
    for (item_code, tipo_vendita), items in item_groups.items():
        adjustment = calculate_group_minimum(
            items, 
            item_code, 
            tipo_vendita, 
            customer_group
...
def update_document_notes(doc, item_groups, customer_group):
    ...
    for (item_code, tipo_vendita), items in item_groups.items():
        ...
        for item in items:
            ...
            original_amount = flt(item.qty) * (flt(item.amount) / flt(item.qty) - flt(item.manual_rate_override or 0))
            total_adjustment += flt(item.amount) - original_amount
        
        if total_adjustment > 0:
            unit = "m²" if tipo_vendita == "Metro Quadrato" else "ml" if tipo_vendita == "Metro Lineare" else "pz"
...
def apply_per_row_minimums(doc, customer_group):
    ...
    for item in doc.items:
        ...
        minimum_config = get_minimum_config(
            item.item_code, 
            tipo_vendita, 
            customer_group
        )
...
def toggle_global_minimums(enabled=True):
    ...
    return {
        "success": True,
        "enabled": enabled,
        "message": _("Minimi globali {}").format(
            _("abilitati") if enabled else _("disabilitati")
        )
    }
...
def get_minimum_summary(doctype, docname):
    ...
    return {
        "customer_group": customer_group,
        "use_global": frappe.db.get_single_value("Selling Settings", "use_global_minimums") or True,
...
def analyze_document_minimums(doc, customer_group):
    ...
    for item_code, items in item_groups.items():
        # Qui implementare analisi dettagliata
        # Per ora restituiamo struttura base
        results.append({
            "item_code": item_code,
            "rows": len(items),
            "has_minimum": False  # TODO: implementare check
        })
